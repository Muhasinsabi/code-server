...
import { IProductConfiguration, IServerRequestWorkbenchConstructionOptions, IServerWorkbenchConstructionOptions, IWorkspace } from 'vs/workbench/workbench.web.api';
...
export interface IEnvironmentServerService extends INativeEnvironmentService {
	...
	createWorkbenchConstructionOptions: (requestOptions: IServerRequestWorkbenchConstructionOptions, startPath: ServerConfiguration['startPath']) => Promise<IServerWorkbenchConstructionOptions>;
}
...
export class EnvironmentServerService extends NativeEnvironmentService implements IEnvironmentServerService {
	...
	public async createWorkbenchConstructionOptions(requestOptions: IServerRequestWorkbenchConstructionOptions, startPath: ServerConfiguration['startPath']): Promise<IServerWorkbenchConstructionOptions> {
		...
		logoutEndpointUrl.pathname = path.join(requestOptions.base, '/logout') + `?base=${requestOptions.base}`;

		const webEndpointUrl = new URL(this.configuration.serverUrl.toString());
		webEndpointUrl.pathname = path.join(requestOptions.base, '/static');
		...
		if (!this.configuration.disableUpdateCheck) {
			productConfiguration.updateUrl = path.join(requestOptions.base, '/update/check');
		}
