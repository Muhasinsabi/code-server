...
export class ServerProcessMain extends Disposable {
	...
	public async startup(): Promise<NetServer> {
		// Services
		const [instantiationService, environmentServerService, logService, bufferLogService] = await this.createServices();

		// Net Server
		const serverUrl = new URL(`http://${this.configuration.remoteAuthority}`);
		const netServer = createNetServer();

		netServer.on('request', async (req, res) => {
			const { csStaticBase, base } = this.configuration;
			// webEndpointUrl.pathname = path.join(csStaticBase, 'lib/vscode');

			const workbenchConstructionOptions = await environmentServerService.createWorkbenchConstructionOptions({ csStaticBase, base }, this.configuration.startPath);
			defaultRequestHandler(req, res, workbenchConstructionOptions);
		});

		const websocketServer = new WebSocketHandler(netServer, environmentServerService, logService);

		// IPC Server
		const ipcServer = new IPCServer<RemoteAgentConnectionContext>(websocketServer.onDidClientConnect);
		...
		this._register(combinedDisposable(instantiationService.createInstance(LogsDataCleaner), instantiationService.createInstance(ErrorTelemetry), websocketServer, ipcServer));

		// Listen for connections
		await listen(netServer, parseInt(serverUrl.port, 10), serverUrl.hostname);
		// netServer.on('error', error => logService.error('[Server Process Error]', error));
			...
			telemetryService = new TelemetryService(
				{
					...
					commonProperties: resolveCommonProperties(fileService, release(), hostname(), process.arch, environmentServerService.commit, product.version, machineId, undefined, environmentServerService.installSourcePath, 'code-server'),
